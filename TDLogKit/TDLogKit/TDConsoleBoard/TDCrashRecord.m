...
+ (void)showCrashInfoWithMoreInfo:(NSString *)moreInfo {
    ...
    NSString *title = [NSString stringWithFormat:@"崩溃报告:%@",[TDCrashRecord backStringWithFormatString:@"yyyy-MM-dd HH:mm" timestampString:time]];
    NSString *str = [NSString stringWithFormat:@"========环境========\n%@\n%@%@",[TDCrashRecord envirDetailInfo],moreInfo?[NSString stringWithFormat:@"%@\n",moreInfo]:@"",info];
    [TDAlertUtil presentAlertWithTitle:title message:str actionTitles:@[@"知道了",@"复制"] preferredStyle:UIAlertControllerStyleAlert handler:^(NSUInteger buttonIndex, NSString *buttonTitle) {
...
void UncaughtExceptionHandler(NSException *exception){
    ...
    [TDCrashRecord recordCrashInfo:crashDic];
}
...
+ (NSString *)envirDetailInfo {
    ...
    //手机型号
    [arr addObject:[NSString stringWithFormat:@"手机型号:%@",[TDCrashRecord currentModel]]];
    return [arr componentsJoinedByString:@"\n"];
}


+ (NSString *)currentModel {
    return (NSString *)[[TDDeviceDataLibrery sharedLibrery] getDiviceName];
}

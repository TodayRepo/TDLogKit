...
#define TDConsoleBoardOriginWidth SCREEN_WIDTH / MaxConsoleWidthSCale
#define TDConsoleBoardOriginHeight SCREEN_HEIGHT / MaxConsoleHeightSCale
...
@property(nonatomic, strong) NSAttributedString *historyLogs;

@end

@implementation TDConsoleBoard

+ (instancetype)borad {
    static TDConsoleBoard* _board;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _board = [[TDConsoleBoard alloc] init];
    });
    return _board;
}
...
- (void)addLog:(NSNotification *)noti {
    ...
    NSString *timeStr = [NSString stringWithFormat:@"%ld.%ld.%ld  %ld:%ld:%ld",comp.year,comp.month,comp.day,comp.hour,comp.minute,comp.second];

    NSLog(@"%@",noti.object);
    
    NSString *text  = [NSString stringWithFormat:@"\n%@\r%@",timeStr,noti.object];
    dispatch_async(dispatch_get_main_queue(), ^{
        //self.historyLogs   self.textView.attributedText
        NSMutableAttributedString *totalText = [[NSMutableAttributedString alloc] initWithAttributedString:self.historyLogs];
...
- (void)createBoard {
    ...
    _contentView.frame = CGRectMake( 0, SCREEN_HEIGHT/2,TDConsoleBoardOriginWidth,TDConsoleBoardOriginHeight);
    _contentView.clipsToBounds = NO;
    _toolView =  [[UIView alloc] init];
    _toolView.backgroundColor = RGBA(230, 230, 230, 0.9);
    
    [[self keyWindow] addSubview:_toolView];
...
//    [_closeBtn setImage:IMG_NAME(@"xkconsoleRes.bundle/xk_icon_welfaregoods_detail_close") forState:UIControlStateNormal];
    ...
    [_closeBtn setTitleColor:RGBGRAY(51) forState:UIControlStateNormal];
    ...
    [clean setTitle:@"🆑" forState:UIControlStateNormal];
    [clean setTitleColor:RGBGRAY(51) forState:UIControlStateNormal];
    clean.titleLabel.font = [UIFont systemFontOfSize:14];
//    clean.tintColor = RGBGRAY(51);
//    [clean setImage:IMG_NAME(@"xk_btn_coinDeail_timeDelete") forState:UIControlStateNormal];
...
- (void)show {
//#if DEBUG
    if (self.historyLogs.length) {
        self.textView.attributedText = self.historyLogs;
        [self.textView scrollRangeToVisible:NSMakeRange(self.historyLogs.length - 2, 1)];
    }
    
    [[self keyWindow] addSubview:_contentView];
//#endif
}
...
-(void)dragArrow:(UIPanGestureRecognizer *)rec {
    ...
    if (rec.state == UIGestureRecognizerStateBegan) {
        ...
        CGFloat width =  MIN(MAX(self.contentView.frame.size.width + point.x, MinConsoleWidth), TDConsoleBoardOriginWidth * MaxConsoleWidthSCale);
        CGFloat height = MIN(MAX(self.contentView.frame.size.height + point.y, MinConsoleHeight), TDConsoleBoardOriginHeight * MaxConsoleHeightSCale);
...
- (void)handlePinch:(UIPinchGestureRecognizer *)pinchGestureRecognizer
{
    ...
    if (pinchGestureRecognizer.state == UIGestureRecognizerStateBegan || pinchGestureRecognizer.state == UIGestureRecognizerStateChanged) {
        ...
        view.frame = CGRectMake(0, 0, MIN(MAX(view.frame.size.width * scale, MinConsoleWidth), TDConsoleBoardOriginWidth * MaxConsoleWidthSCale)  , MIN(MAX(view.frame.size.height * scale, MinConsoleHeight), TDConsoleBoardOriginHeight * MaxConsoleHeightSCale));
        view.center = center;
